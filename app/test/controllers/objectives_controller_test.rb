...
class ObjectivesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create objective" do
    '''#assert_difference('Objective.count') do
      '''post objectives_url, params: { objective: { date: @objective.date, name: @objective.name, status: @objective.status } }
    end
  ...
  test "should update objective" do
    patch objective_url(@objective), params: { objective: { date: @objective.date, name: @objective.name, status: @objective.status } }
